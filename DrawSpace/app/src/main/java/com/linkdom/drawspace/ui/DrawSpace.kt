...
fun DrawSpace(
    ...
    var prevX by remember { mutableStateOf(0f) }  // 用于保存上一个点的横坐标
    var prevY by remember { mutableStateOf(0f) }  // 用于保存上一个点的纵坐标
                ...
                when (event.action) {
                    ...
                    MotionEvent.ACTION_MOVE -> {  // 当手指移动时
                        ...
                        if (event.x < 0f || event.y < 0f){
                            drawPath = DrawPath.MoveTo(prevX,prevY)
                        }else{
                            drawPath = DrawPath.CurveTo(prevX, prevY, event.x, event.y)  // 记录下曲线到的位置
                        }
                        onDrawEvent.invoke(DrawEvent.Move(event.x, event.y))  // 触发移动事件的回调函数

                    }
                    MotionEvent.ACTION_UP -> {  // 当抬起手指时
                        onDrawEvent.invoke(DrawEvent.Up)  // 触发抬起事件的回调函数
                    }
                    else -> { /* do nothing */ }  // 其他情况下不需要做任何操作
                }
                // 输出x,y坐标的方便调试
...
//                path.cubicTo(prevX, prevY, (x + prevX)/2, (y + prevY)/2, x, y)  // 绘制三次贝塞尔曲线
        ...
        drawPath(  // 绘制当前的画笔轨迹
            path = path,
            color = DigitalInkColors.Stroke,  // 绘制轨迹的颜色
            style = Stroke(width = 5f, cap = StrokeCap.Round, join = StrokeJoin.Round)  // 绘制轨迹的样式
        )
    }
}